package com.example.fileexportjt.export_feature.presentation
...
import com.example.fileexportjt.export_feature.presentation.state.FileExportState
...
@HiltViewModel
class ExportViewModel @Inject constructor(private val exportRepository: ExportRepository) :
    ViewModel() {
    ...
    fun generateExportFile() {
        ...
        exportRepository.startExportData(exportList.toList()).onEach { pathInfo ->
            when (pathInfo) {
                is Resource.Success -> {
                ...
                is Resource.Loading -> {
                    pathInfo.data?.let {
                        fileExportState =
                            fileExportState.copy(generatingProgress = pathInfo.data.progressPercentage)
                    }
                }
                is Resource.Error -> {
                    fileExportState =
                        fileExportState.copy(failedGenerating = true, isGeneratingLoading = false)
                }
