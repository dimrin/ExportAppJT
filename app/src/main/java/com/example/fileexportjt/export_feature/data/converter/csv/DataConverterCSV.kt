package com.example.fileexportjt.export_feature.data.converter.csv

import com.example.fileexportjt.core.Resource
import com.example.fileexportjt.export_feature.data.converter.DataConverter
import com.example.fileexportjt.export_feature.data.converter.GenerateInfo
...
class DataConverterCSV : DataConverter {
    ...
    private fun getCSVWriter(writer: Writer): ICSVWriter {
        return CSVWriterBuilder(writer).withSeparator(SEPARATOR)
            .withQuoteChar(CSVWriter.NO_QUOTE_CHARACTER)
            .withEscapeChar(CSVWriter.DEFAULT_ESCAPE_CHARACTER)
            .withLineEnd(CSVWriter.DEFAULT_LINE_END).build()
    }

    override fun convertSensorData(exportDataList: List<ExportModel>): Flow<Resource<GenerateInfo>> =
        flow {
            ...
            exportDataList.forEach { exportModel ->
                ...
                if (alreadyConvertedValues % valuesForOnePercent == 0) {
                    emit(Resource.Loading(GenerateInfo(progressPercentage = alreadyConvertedValues / valuesForOnePercent)))
                }
